...
internal static unsafe class XxHashShared
{
    ...
    static XxHashShared()
    {
        ...
        var secret = stackalloc byte[SecretLengthBytes];
        ...
        var defaultSecretUInt64 = MemoryMarshal.Cast<byte, ulong>(DefaultSecret);
        ...
        var defaultSecret3UInt64 = MemoryMarshal.Cast<byte, ulong>(DefaultSecret.Slice(3));
    ...
    public static void ConsumeStripes(ulong* accumulators, ref ulong stripesSoFar, ulong stripesPerBlock, byte* source, ulong stripes, byte* secret)
    {
        ...
        var stripesToEndOfBlock = stripesPerBlock - stripesSoFar;
    ...
    public static void Append(ref State state, ReadOnlySpan<byte> source)
    {
        ...
        fixed (byte* buffer = state.Buffer)
        {
            ...
            fixed (byte* sourcePtr = &MemoryMarshal.GetReference(source))
            {
                ...
                {
                    var loadSize = InternalBufferLengthBytes - (int)state.BufferedCount;
                ...
                {
                    var stripes = (ulong)(source.Length - sourceIndex - 1) / StripeLengthBytes;
                    Debug.Assert(NumStripesPerBlock >= state.StripesProcessedInCurrentBlock);

                    // Join to current block's end.
                    var stripesToEnd = NumStripesPerBlock - state.StripesProcessedInCurrentBlock;
    ...
    public static void DigestLong(ref State state, ulong* accumulators, byte* secret)
    {
        ...
        fixed (byte* buffer = state.Buffer)
        {
            ...
            {
                var stripes = (state.BufferedCount - 1) / StripeLengthBytes;
                var stripesSoFar = state.StripesProcessedInCurrentBlock;
            ...
            {
                var lastStripe = stackalloc byte[StripeLengthBytes];
                var catchupSize = StripeLengthBytes - (int)state.BufferedCount;
    ...
    {
        var upper = Multiply64To128(left, right, out var lower);
        return lower ^ upper;
    }
    ...
    public static void DeriveSecretFromSeed(byte* destinationSecret, ulong seed)
    {
        fixed (byte* defaultSecret = &MemoryMarshal.GetReference(DefaultSecret))
        {
            ...
            if (Vector256.IsHardwareAccelerated && BitConverter.IsLittleEndian)
            {
                ...
                for (var i = 0; i < SecretLengthBytes; i += Vector256<byte>.Count)
                {
            ...
            else if (Vector128.IsHardwareAccelerated && BitConverter.IsLittleEndian)
            {
                ...
                for (var i = 0; i < SecretLengthBytes; i += Vector128<byte>.Count)
                {
...
#endif
            {
                for (var i = 0; i < SecretLengthBytes; i += sizeof(ulong) * 2)
                {
    ...
    private static void Accumulate(ulong* accumulators, byte* source, byte* secret, int stripesToProcess, bool scramble = false, int blockCount = 1)
    {
        ...
        if (Vector256.IsHardwareAccelerated && BitConverter.IsLittleEndian)
        {
            ...
            for (var j = 0; j < blockCount; j++)
            {
                ...
                for (var i = 0; i < stripesToProcess; i++)
                {
    ...
    private static void Accumulate512Inlined(ulong* accumulators, byte* source, byte* secret)
    {
        ...
        if (Vector256.IsHardwareAccelerated && BitConverter.IsLittleEndian)
        {
            for (var i = 0; i < AccumulatorCount / Vector256<ulong>.Count; i++)
            {
                var accVec = Accumulate256(Vector256.Load(accumulators), source, Vector256.Load((uint*)secret));
        ...
        {
            for (var i = 0; i < AccumulatorCount / Vector128<ulong>.Count; i++)
            {
                var accVec = Accumulate128(Vector128.Load(accumulators), source, Vector128.Load((uint*)secret));
...
#endif
        {
            ...
            {
                var sourceVal = ReadUInt64LE(source + (8 * i));
                var sourceKey = sourceVal ^ ReadUInt64LE(secret + (i * 8));
    ...
    private static Vector256<ulong> Accumulate256(Vector256<ulong> accVec, byte* source, Vector256<uint> secret)
    {
        ...
        var sourceKey = sourceVec ^ secret;
        ...
        var sum = accVec + sourceSwap.AsUInt64();
        var product = Avx2.IsSupported ?
    ...
    private static Vector128<ulong> Accumulate128(Vector128<ulong> accVec, byte* source, Vector128<uint> secret)
    {
        ...
        var product = MultiplyWideningLower(sourceKey);
        accVec = product + sum;
        return accVec;
    }
    ...
    private static Vector128<ulong> MultiplyWideningLower(Vector128<uint> source)
    {
        ...
        {
            var sourceLow = Vector128.Shuffle(source, Vector128.Create(0u, 2, 0, 0)).GetLower();
            var sourceHigh = Vector128.Shuffle(source, Vector128.Create(1u, 3, 0, 0)).GetLower();
    ...
    private static void ScrambleAccumulators(ulong* accumulators, byte* secret)
    {
        ...
        if (Vector256.IsHardwareAccelerated && BitConverter.IsLittleEndian)
        {
            for (var i = 0; i < AccumulatorCount / Vector256<ulong>.Count; i++)
            {
                var accVec = ScrambleAccumulator256(Vector256.Load(accumulators), Vector256.Load((ulong*)secret));
                Vector256.Store(accVec, accumulators);
        ...
        else if (Vector128.IsHardwareAccelerated && BitConverter.IsLittleEndian)
        {
            for (var i = 0; i < AccumulatorCount / Vector128<ulong>.Count; i++)
            {
                var accVec = ScrambleAccumulator128(Vector128.Load(accumulators), Vector128.Load((ulong*)secret));
                Vector128.Store(accVec, accumulators);
...
#endif
        {
            for (var i = 0; i < AccumulatorCount; i++)
            {
                var xorShift = XorShift(*accumulators, 47);
                var xorWithKey = xorShift ^ ReadUInt64LE(secret);
    ...
    {
        var xorShift = accVec ^ Vector256.ShiftRightLogical(accVec, 47);
        var xorWithKey = xorShift ^ secret;
    ...
    {
        var xorShift = accVec ^ Vector128.ShiftRightLogical(accVec, 47);
        var xorWithKey = xorShift ^ secret;
