...
internal struct MemoryBuilder : IDisposable
{
    byte[] _array;
    ...
    {
        _array = ArrayPool<byte>.Shared.Rent(512);
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public MemoryBuilder(int initialCapacity)
    {
        Debug.Assert(initialCapacity > 0);
    ...
    public readonly Memory<byte> Written
    {
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        get => _array.AsMemory(0, BytesWritten);
    }

    public readonly Span<byte> Free
    {
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
